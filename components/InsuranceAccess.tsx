  ...
  if (!insurance) {
    return (
      <div className="max-w-4xl mx-auto py-20 animate-in fade-in duration-500">
        <div className="bg-white dark:bg-slate-900 rounded-[3rem] p-12 text-center shadow-2xl border border-slate-100 dark:border-slate-800">
          ...
          <h2 className="text-3xl font-black text-slate-900 dark:text-white mb-4 uppercase tracking-tight">Insurance Setup</h2>
          <p className="text-slate-500 dark:text-slate-400 mb-10 max-w-lg mx-auto font-medium">Link your insurance plan to unlock smart cost tracking, prior-auth management, and automated appeal letters.</p>
          <button
            onClick={() => {
              setEditForm(emptyInsuranceState);
              setShowEditPlan(true);
            }}
            className="px-8 py-5 bg-medical-blue text-white rounded-2xl font-black uppercase tracking-widest text-xs hover:scale-105 transition-all shadow-xl shadow-blue-500/30"
          >
            + Add Insurance Details
          </button>
        </div>

        {/* Re-use the existing modal for the initial setup */}
        {showEditPlan && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-6 text-left">
            ...
            <div className="relative bg-white dark:bg-slate-900 w-full max-w-xl rounded-[3rem] p-10 shadow-2xl animate-in zoom-in-95 duration-300">
              <div className="flex justify-between items-center mb-8">
                <h3 className="text-2xl font-black text-slate-800 dark:text-white uppercase tracking-tighter">Setup Plan Details</h3>
                <button onClick={() => setShowEditPlan(false)} className="p-2 bg-slate-50 dark:bg-slate-800 rounded-full"><X size={18} /></button>
              </div>
              <form onSubmit={handleSavePlan} className="space-y-6">
                {/* Reuse the same form fields structure but for initial setup */}
                ...
                <div className="pt-6 border-t border-slate-100 dark:border-slate-800">
                  <button type="submit" className="w-full py-5 bg-medical-blue text-white rounded-[1.75rem] font-black uppercase tracking-widest text-[10px] shadow-xl shadow-blue-500/20 flex items-center justify-center gap-2">
                    <Save size={18} /> Save Initial Plan
                  </button>
                </div>
              </form>
            </div>
          ...
          <div className="bg-white dark:bg-slate-900 p-10 rounded-[3rem] border border-slate-100 dark:border-slate-800 shadow-xl relative overflow-hidden">
            ...
            <div className="relative">
              <div className="flex items-center justify-between mb-10">
                ...
                <span className={`px-4 py-1.5 text-[10px] font-black uppercase tracking-widest rounded-full border ${insurance.status === 'Active' ? 'bg-green-100 text-green-700 border-green-200' : 'bg-amber-100 text-amber-700 border-amber-200'
