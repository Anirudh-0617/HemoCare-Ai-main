...
import { Plus, MapPin, Camera, Info, Search, Filter, Clock, ChevronDown, X, Trash2, Check, Droplets, Activity, ShoppingBag, MoveLeft, MoveRight } from 'lucide-react';
...
const BleedTracker: React.FC<Props> = ({ bleeds, onAddBleed, onNavigate }) => {
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-white dark:bg-slate-900 p-6 md:p-10 rounded-[2.5rem] border border-blue-100 dark:border-slate-800 shadow-2xl space-y-8 animate-in zoom-in-95 duration-300">
          <div className="flex items-center justify-between">
            <h3 className="text-lg font-black text-slate-800 dark:text-white uppercase tracking-tighter">New Entry</h3>
          ...
          <div className="pt-6 border-t border-slate-50 dark:border-slate-800 flex gap-3">
            ...
            <button type="submit" className="flex-1 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest bg-blue-600 text-white shadow-xl shadow-blue-100 active:scale-95 transition-all">Save Episode</button>
          </div>
        </form>
      )}
